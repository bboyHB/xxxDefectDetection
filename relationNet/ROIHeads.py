...
class RoIHeads(torch.nn.Module):
    ...
    def forward(self,
        ...
        # 结合全局特征，整张图做ROI POOLing和各个ROI相加
        global_boxes = [
            torch.tensor([[0, 0, image_shape[1], image_shape[0]]] * proposals[index].shape[0], dtype=torch.float, device=proposals[0].device)
            for index, image_shape in enumerate(image_shapes)]
        global_features = self.box_roi_pool(features, global_boxes, image_shapes)
        global_features = self.box_head(global_features)
        box_features = box_features + global_features

        class_logits, box_regression = self.box_predictor(box_features, proposals)
...
def get_ROIHeads(num_classes):
    ...
    box_head = TwoMLPHead(
        ...
        representation_size)
    representation_size = 1024
    box_predictor = RelationPredictor(
        representation_size,
        num_classes)
